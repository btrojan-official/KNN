...
with open("results.txt", "a", encoding="utf-8") as f:
    test_id = 1
    test_num = len(_weights) * len(_k) * len(_lambda) * len(_lambda) * len(_tukey) * len(_means)
                    ...
                    for tukey_lambda in _tukey:
                        for kmeans in _means:
                            try:
                                ...
                                message = f"{test_id}/{test_num} w = {weight}, k = {k}, l1 = {knn.l1}, l2 = {knn.l2}, tukey = {knn.apply_tukeys_transformation}, tukey_lambda = {knn.lambda_hyperparameter}, kmeans: {kmeans}, accuracy = {torch.sum((y_test.flatten().to(device)==predictions).int()).double() / X_test.shape[0] * 100}"
                                ...
                                print()
                                message = f"{test_id}/{test_num} ERROR w = {weight}, k = {k}, l1 = {knn.l1}, l2 = {knn.l2}, tukey = {knn.apply_tukeys_transformation}, tukey_lambda = {knn.lambda_hyperparameter}, kmeans: {kmeans}, accuracy = ERROR"
                                k_values.append(k)
                                l1_values.append(l1)
                                l2_values.append(l2)
                                tukey_values.append(tukey_lambda)
